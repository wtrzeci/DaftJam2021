...
namespace Elympics
{
	internal class DynamicElympicsBehaviourInstancesData : ElympicsVar
	{
		...
		private readonly Dictionary<int, byte[]> _instancesSerialized         = new Dictionary<int, byte[]>();
		private readonly Dictionary<int, byte[]> _incomingInstancesSerialized = new Dictionary<int, byte[]>();

		private readonly List<(int, byte[])> _instancesToRemoveSerialized = new List<(int, byte[])>();
		private readonly List<(int, byte[])> _instancesToAddSerialized    = new List<(int, byte[])>();

		private readonly Dictionary<int, byte[]> _equalsInstancesSerialized1 = new Dictionary<int, byte[]>();
		private readonly Dictionary<int, byte[]> _equalsInstancesSerialized2 = new Dictionary<int, byte[]>();

		public DynamicElympicsBehaviourInstancesData() : base(true)
		{
		}
		...
		internal void ApplyIncomingInstances()
		{
			...
			foreach (var (instanceId, _) in _instancesToRemoveSerialized)
				_instancesSerialized.Remove(instanceId);
