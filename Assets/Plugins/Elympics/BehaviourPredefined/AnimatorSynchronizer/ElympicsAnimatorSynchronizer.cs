...
namespace Elympics
{
	...
	public class ElympicsAnimatorSynchronizer : ElympicsMonoBehaviour, IInitializable, IStateSerializationHandler
	{
		...
		private bool ShowLayerWeights = true;

		[SerializeField, HideInInspector]
		private bool ShowParameters = true;

		[SerializeField, HideInInspector]
		private List<ParameterSynchronizationStatus> _boolStatuses = new List<ParameterSynchronizationStatus>();

		[SerializeField, HideInInspector]
		private List<ParameterSynchronizationStatus> _intStatuses = new List<ParameterSynchronizationStatus>();

		[SerializeField, HideInInspector]
		private List<ParameterSynchronizationStatus> _floatStatuses = new List<ParameterSynchronizationStatus>();

		[SerializeField, HideInInspector]
		private List<ParameterSynchronizationStatus> _triggersStatuses = new List<ParameterSynchronizationStatus>();

		[SerializeField, HideInInspector]
		private List<LayerSynchronizationStatus>     _layersStatuses = new List<LayerSynchronizationStatus>();
		...
		public bool GetLayer(int layerIndex)
		{
			...
			if (index == -1)
			{
				_layersStatuses.Add(new LayerSynchronizationStatus { Enabled = true, Index = layerIndex});
				return true;
			}
		...
		public bool GetParameter(AnimatorControllerParameterType type, int hashName)
		{
			...
			if (index == -1)
			{
				statuses.Add(new ParameterSynchronizationStatus {Enabled = true, HashName = hashName});
				return true;
			}
